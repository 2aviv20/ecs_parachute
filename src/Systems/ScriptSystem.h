...
void SetProjectileVelocity(Entity entity, double x, double y) {
 ...
 void SetSpriteAndAnimation(Entity entity,std::string textureId){
    // Logger::Log(std::to_string(""));
    if (entity.HasComponent<SpriteComponent>() && entity.HasComponent<AnimationComponent>()) {
        // auto& animation = ScriptSystem::assetStore->GetAnimation(textureId);
        ...
        //     };
        // }else{
        //     Logger::Err("entity has no animation component or sprite component");
        }
}

// void AddForce(Entity entity, float forceX, float forceY){
void AddForce(Entity entity, float forceX, float forceY){
    if (entity.HasComponent<RigidBodyComponent>()) {
        auto& rigidBody = entity.GetComponent<RigidBodyComponent>();
        rigidBody.addForce(glm::vec2(forceX,forceY));
    } else {
        Logger::Err("Trying to add force to an entity that has no Force component");
    }
}
...
void ClearForces(Entity entity){
    if (entity.HasComponent<RigidBodyComponent>()) {
        ...
        Logger::Err("Trying to clear force from an entity that has no Force component");
    }
}
...
std::string GetAssetId(Entity entity){
    if (entity.HasComponent<SpriteComponent>()) {
        auto& sprite = entity.GetComponent<SpriteComponent>();
        return sprite.assetId;
    } else {
        Logger::Err("Trying to get assetId from an entity that has no SpriteComponent");
    }
    return "";
}
class ScriptSystem: public System {
    public:
        ...
        void CreateLuaBindings(sol::state& lua) {
            ...
            lua.set_function("get_assetId", GetAssetId);
            ...
            lua.set_function("set_sprite_and_animation", SetSpriteAndAnimation);
            lua.set_function("force_add_force", AddForce);
            lua.set_function("force_clear_force", ClearForces);
