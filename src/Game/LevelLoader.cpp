...
void LevelLoader::LoadLevel(sol::state& lua, std::shared_ptr<Registry> registry, const std::shared_ptr<AssetStore>& assetStore, SDL_Renderer* renderer, int levelNumber) {
        ...
        if (hasComponents != sol::nullopt) {
            ...
            if (transform != sol::nullopt) {
                newEntity.AddComponent<TransformComponent>(
                    glm::vec2(
                        entity["components"]["transform"]["position"]["x"].get_or(50),
                        entity["components"]["transform"]["position"]["y"].get_or(50)
                    ),
                    glm::vec2(
            ...
            if (rigidbody != sol::nullopt) {
                newEntity.AddComponent<RigidBodyComponent>(
                    glm::vec2(
                     ...
                     entity["components"]["rigidbody"]["mass"]
                );
            }
            ...
            if (animation != sol::nullopt) {
                newEntity.AddComponent<AnimationComponent>(
                    ...
                    entity["components"]["animation"]["horizontal"].get_or(true)
                );
            }
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Read the level animations 
    ////////////////////////////////////////////////////////////////////////////
    sol::table animations = level["animations"];
    i = 0;
    while (true) {
        sol::optional<sol::table> hasAnimation = animations[i];
        if (hasAnimation == sol::nullopt) {
            break;
        }

        sol::table animation = animations[i];
        std::string textureId = animation["textureId"];
        int num_frames  = animation["num_frames"];
        int speed_rate  = animation["speed_rate"];
        bool horizontal = animation["horizontal"];
        int width = animation["width"];
        int height = animation["height"];
        assetStore->AddAnimation(textureId,num_frames,speed_rate,horizontal, width, height);
